...
export const config = {
  ...
  maxStories: numberEnv("MAX_STORIES", 10),
  maxStoryDiscoveryPages: numberEnv("MAX_STORY_DISCOVERY_PAGES", 32),
  maxInspiration: numberEnv("MAX_INSPIRATION", 5),
  maxInspirationDiscoveryPages: numberEnv("MAX_INSPIRATION_DISCOVERY_PAGES", 32),
  maxDiscoveryPages: numberEnv("MAX_DISCOVERY_PAGES", 32),
  maxDetailPagesPerSource: numberEnv("MAX_DETAIL_PAGES_PER_SOURCE", 16),
  minQualityScore: numberEnv("MIN_QUALITY_SCORE", 0.68),
  minInspirationQualityScore: numberEnv("MIN_INSPIRATION_QUALITY_SCORE", 0.82),
  refParam: process.env.REF_PARAM ?? "dzinerhub.com",
  sourceFile: process.env.SOURCE_FILE ?? "data/sources.json",
  storySourceFile: process.env.STORY_SOURCE_FILE ?? "data/story-sources.json",
